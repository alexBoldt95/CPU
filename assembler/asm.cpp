...
struct instruction
{
      ...
      case R0:
        if(opstr == "halt")
        {
          ...
          *ptr = *opcodes["beq"];
          ...
          ptr->next_instruction = new instruction(*opcodes["beq"]);
          ptr = ptr->next_instruction;
          ptr->insn.itype.rs = 0;
          ptr->insn.itype.rt = 0;
          ptr->insn.itype.imm = -1;
  ...
  void fill_in_symbols()
  {
    if(immed_symbol != "")
    {
      ...
      {
        char buf[512];
        sprintf(buf,"3Immediate value out of range [%d,%d] on line %d: %d",MIN_IMM,MAX_IMM,lineno,imm);
        throw string(buf);
      }
      insn.itype.imm = imm;
    }
...
int main(int argc, char** argv)
{
  ...
  // pseudo-instructions
  opcodes["ldia"] = new instruction(0,"ldia",IBranch);
...
void usage()
{
  cerr << "Usage:" << endl
       ...
       << "  inputfile.imem.lgsim: memory initialization file for the" << endl
       << "                      hardware-implementation instruction memory" << endl
       << "  inputfile.dmem.lgsim: memory initialization file for the" << endl
